...
| Case2 | 当前节点的父节点是红色，叔叔节点是黑色，且当前节点是其父节点的右孩子             | (01) 将“父节点”作为“新的当前节点”。<br>(02) 以“新的当前节点”为支点进行左旋。之后继续对之前设定的“当前节点”进行操作。                                                               |
...
1. **(Case 1)叔叔是红色**
Case 1主要是变色操作，变色操作后，**如果祖父结点是根节点，那么直接将根节点涂为黑色即可！** 否则将祖父结点作为当前节点进行新一轮的操作。在新一轮的操作用可能会转到Case 2和3。
如下图经过Case1调整后，结点60变为当前节点。
![insert1](./img/RBTree/insert1.jpg)
2. **(Case 2)叔叔是黑色，且当前节点是右孩子**
![insert1](./img/RBTree/insert2.jpg)
如上图，经过case调整后结点60为当前节点，然后按照Case2，**将40设为当前结点**进行左旋。注意左旋完成后结点40依然是当前节点。
